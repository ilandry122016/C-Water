This is a program to add the C-water watermark.
...
static void
add_c_water(GimpDrawable* drawable,
              gint lower_limit_x,
              gint lower_limit_y,
              gint upper_limit_x,
              gint upper_limit_y,
    ...
    GimpParam** return_vals)
{
  ...
  *return_vals = values;
  ...
  gimp_progress_init("Add C_Water...");
  ...
  add_c_water(drawable, x1, x2, y1, y2, channels);
...
query(void)
{
  ...
  gimp_install_procedure("plug-in-add-c_water",
                         "add-c_water",
                         "Displays \"Add c_water\" in a dialog",
                         "Isaac Landry",
  ...
  gimp_plugin_menu_register("plug-in-add-c_water", "<Image>/Filters/Misc");
}
              ...
              gint channels)
{
  ...
  // Check if the new_bits can't fit the new c_water.
  if (compressed_size + BLAKE3_OUT_LEN > original_bits_size) {
    // Display a dialog.
    g_message("Could not add the c_water because it will not fit.\n");
    return;
  }

  // The c_water is the hash of all pixels in the image and the bits
